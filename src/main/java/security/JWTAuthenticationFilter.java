...
public class JWTAuthenticationFilter implements ContainerRequestFilter {
 ...
 public void filter(ContainerRequestContext request) throws IOException {
   if (isSecuredResource()) {
     ...
     try {AccountPrincipal account = getAccountPrincipalFromTokenIfValid(token);
       //What if the client had logged out????
       request.setSecurityContext(new JWTSecurityContext(account, request));
 ...
 private AccountPrincipal getAccountPrincipalFromTokenIfValid(String token)
...
//     Long id = (Long) signedJWT.getJWTClaimsSet().getClaim("id");
     ...
     return new AccountPrincipal(username, rolesArray);
